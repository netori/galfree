...
describe('方言子集解析器(T4)', () => {
  it('模板解析:场景数/对白/菜单边结构正确', () => {
    ...
    expect(dialogue).toMatchObject({ kind: 'dialogue', speaker: null, text: '从这里开始你的故事。' })
  ...
  it('常见手改:Character 定义/带说话人对白/show/hide/with/call/独立 jump', () => {
    ...
    expect(start.statements).toContainEqual({ kind: 'dialogue', speaker: 'e', text: '你好呀。', line: 7 })
    expect(start.statements).toContainEqual({ kind: 'dialogue', speaker: null, text: '旁白一句。', line: 8 })
