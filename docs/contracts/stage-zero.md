...
起因是一句真问题:「IndexTTS 2.5 没有设计音色的功能,该如何保持声音一致性?」
调研定稿在 `docs/research-indextts-voice.md`(**全部有行号出处**),三条硬事实决定了这一节的形状:

| 事实 | 出处 |
|---|---|
| **音色只由参考样本决定**:同路径 ⇒ 同 speaker embedding 缓存命中 ⇒ 同一把嗓子 | `indextts/infer_v2_5.py:344-350,725,774-780` |
| **`speaker` 不是音色**:它只选 LoRA 适配器目录(`runs/exp1_<name>/`);本机 `runs/` 是空的 ⇒ 只有 `default` | `app_api.py:168-183` |
| **`audio` 才是音色**:参考样本**文件名**,服务端只在**它自己的 `voices/` 目录**里按名解析(一层,无子目录) | `app_api.py:115,186-189,347-350` |

⇒ 一致性的答案不是"设计音色"(那家服务做不到),而是**每个角色固定一段参考样本、每次都拿它去克隆**。

### 登记簿里的音色档案(`CharacterRecord.voiceProfile`)

```jsonc
{
  "id": "xiao_tang", "name": "小棠", "voice": "xiao_tang",
  "voiceProfile": {
    "sample": "xiao_tang.wav",          // ← **服务端音色库里的文件名**,不是项目内路径
    "speaker": "default",               // ← LoRA 适配器名(**不是音色**);缺省就是 default
    "lang": "ZH",
    "emotion": { "mode": "vector", "vector": [0,0,0,0,0,0,0,1], "weight": 0.65 }
  }
}
```

- **两个命名空间不许混**(`assertCharacterValid` 里拦,写入之前就拒):`sample` / `refSample` 带
  `/`、`\`、盘符、`..` 一律 `character-invalid`。理由很实:服务端只在**它自己的**音色库里找,
  送一个 `game/voice/x.wav` 过去**必然**吃 400 —— 而那种错只会在一**次真合成**(还花了额度)之后才显形。
- **情感四档**是中立说法,适配器才映射成那家服务的整数:`follow`→`emo_control_method: 0`、
  `reference`(=1,配 `emo_ref_audio`)、`vector`(=2,**恰好 8 个数**)、`text`(=3,服务端没开
  `qwen_emo` 时会 400)。**没配就一个情感字段都不发**(服务端缺省本来就是 follow)—— 不替人编一个 0。
- 换音色 = **设定改动**(ADR-0012),不是主观认可;审读戳规则一个字没动。

### 自动携链(与图像的参考链同一个形状)

建语音任务时(`createAudioTask`,purpose = voice):

1. 显式给了 `voiceId` → 当**登记簿 id** 用;
2. 否则按 `dialogueId` 从 `.rpy` 派生**说话人变量** → 反查登记簿的 `voice` 字段
   (口径与 `voiceBatch` 清单**同一套**:显式 `id` 优先,否则 `dialogueIdFor(label, 序号)`);
3. 都没有 → **如实降级**(不是拒绝):任务上落一条 `voice-anchor-missing` 的 `degradation`,
   说清缺的是哪一样、去哪补。**参考样本拿不到时(模型目录也没写 `audio=`)请求根本不会发出去**
   —— 适配器在 `buildRequest` 里就抛:拿空值去撞上游 400 是白花一次额度。
   (模型目录里写了 `audio=` 的那条**会照发**,用模型级缺省的那把嗓子 —— 那正是降级消息承诺的兜底。)

任务上记 `voiceSample` / `voiceSpeaker` / `voiceLang` / `voiceEmotion` + `voiceId`(登记簿 id),
它们是**制作信息**,不是叙述内容(ADR-0003/0009)。

**降级不是死路**(T32 收尾时补的一小段):已经带着样本的任务,重 roll = 同一个请求再发一次
(**一个字节都不改**);而**还没拿到样本**的那条,重 roll 时会**再解析一次** —— 人很可能刚刚去
角色视图补上了档案,这时点它就该成(不然那条任务只能重建,而"降级"本身就成了死路)。
补上之后账本上会记下这次真用的样本,那条 `voice-anchor-missing` 也随之删掉(它已经不成立,
留着就是过期的话)。

### 修掉的两处错位(都在 `audio-adapter-indextts.ts`)

1. **`voiceId` 不再当 `speaker` 发**。前者是"哪把嗓子"(登记簿 id),后者是"哪份 LoRA"
   —— 拿它当 `speaker` 发,只会得到一个**必然 400** 的值(本机 `runs/` 是空的)。
   现在的来源只有两处:任务的音色档案 → 模型目录 `note` 里的 `speaker=` → 缺省 `default`。
2. **不再把项目内路径当"音色库文件名"发**。老写法 `options.audio ?? task.referenceAudio[0]?.path`
   的兜底**必然**被拒(两个命名空间),而且它把真正的病因藏起来。现在拿不到库文件名就**如实拒绝**,
   并把"音色库是服务端自己的 `voices/`、项目里的 `game/voice/…` 与它不是一个命名空间"写进那句话。

守卫在 `src/service/voice-anchor-flow.test.ts`:**同一个角色的两次任务,请求体里的 `audio` 逐字相同**
(跨批次同一把嗓子),以及"没有档案时一个请求都没发出去"。

### 读音色库:`/health` + `/speakers` + `/voices`

- 路由 `POST /api/galfree/audio/voice/library`,工具 `action: "library"`,接缝 `readVoiceLibrary()`
  —— 走**注入的出网端口**,不花额度(三个端点只读清单);
- **逐端点如实报**:本机 TTS 服务各不相同(`/models` 往往根本没有),没答上来的端点进 `problems`,
  **不整体抛** —— 拉不到 `/speakers` 不等于这条路走不通;
- **`/speakers` 是 LoRA 名清单、不是音色清单**:只有 `default` = 没训过说话人模型,
  面板**如实这么说**,不把它渲染成"一个可选音色";
- **面板要读它**:"这个角色要的那段参考**此刻在不在库里**"在角色视图里就看得见
  (`inLibrary` 三种取值:在 / 不在 / **没核对过** —— 第三种绝不说成"没有");
  `health.qwen_emo === false` 时「情感描述文本」那一档**提前标出来**(并禁掉),不等人点了才撞 400
  (`app_api.py:397-399`);
- 库清单(文件名 + 服务端给的**绝对目录**)只记在**内存**里(`libraryReadAt` 说明它多新)
  —— 它是那台服务**此刻**的处境,不是项目的制作信息,写进 `.studio/` 就成了第二份真相。
  建任务时用它标一句"档案要的样本此刻不在库里"。
- `null` = **没核对过**,不是"库里没有"(把"不知道"渲染成"没有"会让人去修一个不存在的问题)。

### 读写口(**没提到的字段就保留**)

`POST /cast/characters/upsert` 是**整条替换**,而面板/工具每一样都是分开改的(改参考链、改嗓子、
改外观)。T32 起这条路由的规矩统一成一句:**body 里没有这个键 = 别动它**;给了键(空数组/空串/`null`)
才动它 —— `voiceProfile: null` = 明确清掉。不兜这一下,"改一次嗓子把参考链冲掉"这类错会**静默发生**。

### 已知边界(这一票**没有**覆盖的,别当成"已经管了")

1. **没有上传接口**:`voices/` 只能**人把文件丢进去**(服务端只有 `os.listdir`)。所以面板做的是
   "告诉你库在哪、里面有什么",不是"帮你把项目里的 wav 送过去"。想在项目侧闭环就分批**走
   「语音批量清单」那条路**(T29:导出 → 本地工具 → 按 id 导回)。
2. **逐句波形不可复现**(服务端无 seed 参数),但**音色不漂**(同路径 ⇒ 同缓存条件,机制推导,
   官方没有 run-to-run 方差的定量数据)。
3. **`emo_control_method: 3`(情感描述文本)要服务端开 `--qwen_emo`**,否则 400 ——
   面板先把 `qwen_emo: false` 如实显示出来(读音色库的 `health` 里有),不等人点了才撞。
4. **LoRA 那条路没实测过**:本机 `runs/` 与 `finetune_data/` 都是空的,`/speakers` 从未有过
   `default` 以外的值;`speaker` 字段因此只作"将来能训"的位置留着。
5. **同机前提不变**:适配器读服务端给的**绝对路径**拿产物,跨机请走批量清单那条路(T29)。

## 建音频任务的入口(T33 / #41 之后追加)

**缺口是这么来的**:T28/T29 把音乐与语音的适配器、任务队列、落盘、面板卡都做通了,
却**没有人能"建"一条音频任务** —— 22 个 agent 工具里没有它,面板两张卡只有「跑队列 / 重 roll」,
客户端那个 `createAudioTask` 方法**没有任何组件在调**(死代码)。于是"自动加音乐"卡在最后一步:
agent 能读渠道、能接线,但没有手去按下"生成这首曲子"。

### 一条入口,两种产物

`galfree_generate_audio`(agent)与两张卡上的「新建任务」(面板)**同一条写路**:
都落到 `service.createAudioTask`,都进同一份 `.studio/audio-tasks.json`(慢带有守卫:
面板建一条 + 工具建一条 → 同一份账本 → 一次「跑队列」把两条都跑掉)。

- **用途按目标路径判**(`purposeOfPath`):`game/voice/` 下 = 语音,其余 = 音乐 ——
  与接缝**同一处判断**。让调用方声明用途迟早会分叉(声明漏了就走错渠道,而两条渠道的目录不通用);
  所以 T33 起调用方**不再递** `purpose`:真递了就必须与路径一致,不一致当场 `invalid-request`
  (认不出的值如 `'sfx'` 同样如实拒,不静默丢);
- 语音那条:给了 `dialogue_id` 就按台词派生的说话人**自动携音色档案**(T32 的声音锚);
- `run` 的判定**只在接缝那一处**(T33 顺带修的):此前 `CreateAudioTaskInput.run` 只是文档里写着,
  接缝根本没读,面板那条路由自己补跑了一次 —— 于是"面板建的会跑、工具建的只入队"。
  现在路由把标志原样递下去(201 = 只建了 / 200 = 已经跑过一次)。

### 队列的读写口:`galfree_audio_queue`(list / run / retry)

建了要能看、能跑、能重来 —— 与图像那对工具(`galfree_art_queue` + `galfree_reroll_image`)同形,
**成本就是这一层的意义**:

- `action: "list"`(缺省)= 任务清单(状态、目标路径、模型、这条用的音色、末次错误、拒收注记),
  外加 `queued: {music, voice}` —— **两个用途分开数**(两张卡各跑各的,合起来数就会漏)。
  这个数**只有一处实现**(`audio-generation.ts` 的 `countAudioTasks`):面板卡上的"排队 N 条"、
  agent 报的成本、跑队列前那句"这一跑真发几条"读的是同一个函数 ——
  分叉的后果是"面板说的条数 ≠ 真会发出去的条数"(T27 踩过:替另一条渠道花了钱);
- `action: "run"` = 推进排队中的(给 `purpose` 只跑那一类),返回里的 `spend` 那句就是
  "这一跑真发了几条上游请求";
- `action: "retry"` = 重 roll 一条(改词 / 拒收注记):**注记只记人给的原话**(`via: 'agent'` 标出转述),
  没理由就不记;
- **失败不自动重试**:留在账本里等人看(与出图那条同一纪律)。

**成本闸门**(ADR-0012 的原话在面板上落成这样):「新建任务」表单不勾「立刻跑」= 只入队,
勾了按钮就叫「建并跑(1 条请求)」;卡片上那颗「跑队列(N 条)」的 N 就是**这一下真会发出去的条目数**
(数的是**本用途**的,不含另一条渠道)。agent 那侧同一份数字在返回体里 —— 而且是**两个数**:
`cost.attempted`(这一下真跑过几次;0 = 只入队)与 `cost.pending`(此刻排队中、跑队列会真发的条数)。
分两个的理由:`run` 缺省 true 时"这一跑"**已经跑过了**,那时再报队列数就是一句空话。

### 已知边界(这一票**没有**覆盖的)

1. **批量建**(按一场戏一次建 N 条语音 + 那份成本确认)不在本票 —— 它有自己的确认语义,
   值得单独一票(T34 候选:一场戏几十行台词,TTS 按行计费,所以"建"这个动作本身要能一次看清 N);
2. **每任务覆盖"纯音乐 / 带唱"**不在本票:那件事现在由模型目录的能力声明决定
...
**还有一条与协议无关、但同样是为"真金"加的**:异步制提交拿到的**上游任务 id 会记进账本**
(`AudioTask.upstreamTaskId`),失败原因里也带一句 —— 因为"轮询到了终态却认不出产物地址"
意味着**一次真生成已经花掉了**,那个 id 是 48 小时内取回产物的唯一线索(那家网关文档明写 48 小时)。
不记它 = 一句"失败了"把花了钱的东西彻底丢掉。配套的动作是
`galfree_audio_queue(action:"collect")`:**不重新提交**,只按那个 id 问一次状态,成了就下载落盘。

**另一条实测教训(2026-09-13,同一次真机)**:**扩展名不许撒谎**。上游给的是 **mp3**,
而调用方写的目标路径是 `.ogg` —— 项目里就躺了一个 4.6 MB 的"名字说是 ogg、内容是 mp3"的文件,
而 Ren'Py **按扩展名选解码器**,它在游戏里读不出来(又一次"看着生成了、玩的时候没声音")。
现在落盘前**看魔数**(`audioFormatOfBytes`:`ID3` / `OggS` / `RIFF` / `fLaC` / `ftyp`;
content-type 只兜底 —— 很多上游只给 `application/octet-stream`):

- 真实格式 ≠ 目标后缀 ⇒ **换后缀落盘**(`withAudioExtension`)**+ 在任务上记一笔**
  (`degradation.code = "audio-format-mismatch"`:上游给的是 X、路径写的是 .Y、产物落在哪、
  要 .Y 得先转格式);语音那条还会点明"`config.auto_voice` 的模板后缀也要对上,否则引擎找不到它";
- 认不出真实格式 ⇒ **照原路径写**,但同样记一笔(不猜一个后缀);
- 一致时**一个字都不多说**(没有降级说明)。
