...
| **这个 preset 自己** | ① 一段**立场**(persona):galgame 制作现场、按指引走、只报事实、一次推进一环;② 一个**前置检查行**(`guard.mjs`):确认插件在场,不在就**带原因地拒绝**而不是静默少一堆工具 |
...
3. 期望:它**先读板**(调 `galfree_project_status`),再按指引的顺序说话 ——
   先说"要先建项目 / 写设定集,而这两步现在需要你(人)来做",而不是直接开始编剧本;
4. 插件没装时:会话创建**失败**,并指名 `galfree-guard` 这一行,错误里带装法 —— 不是少几个工具。

## 维护

- 本目录的 `agent.cordis.yml` 是随包 `standard` 组装的**逐行副本 + 两处改动**(persona、guard 行)。
  宿主升级 `@deepseek-ai/dsh-agent-presets` 之后,diff 一次
  `node_modules/@deepseek-ai/dsh-agent-presets/presets/standard/agent.cordis.yml`,
  只保留这两处差异。
- `guard.mjs` 里那份"最少要有的工具名"与插件真实注册的名字由仓库测试钉在一起
  (`src/preset.test.ts`):工具改名 → 测试红。
