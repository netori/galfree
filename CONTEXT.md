...
- **环节零(T1–T7)、剧本环节(T8–T13)与图像渠道 + 任务队列(T14)已交付**。issues #9–#22
  已关闭(#22 由 commit 的 `Closes #22` 自动关)。接缝契约成文:
  `docs/contracts/stage-zero.md`(含 T8–T14 各节)+ `dialect-subset.md`。
- 快带 `npm test`(**16 文件 125**)全绿;慢带 `npm run test:slow`(**30**,含真钉版 SDK 的
  模板启动、逐场生成闭环、从某场试玩)在 2026-09-12 跑过并全绿 —— **发版前每次都要重跑**。
- T14 把**写网关拓宽到二进制**(`WriteOp.content: string | Uint8Array | null`),版本戳
  一律按原始字节 —— 这是"素材经网关落盘"这条铁律的前置,不是旁路。
- 遗留:工作台面板的人工验收(重启后看 T8–T14 的卡)**已由本轮真渲染截图回路覆盖渲染面**,
  但"装进真宿主窗口"这一条由用户确认过;素材**动作面**(出图按钮/补全全部/重 roll)属
  **T15**,参考链一致性回路属 **T16**;音频/发布(T17–T18)未开工。
- 下一张票 **T15(出图操作与素材板动作面)**:接缝的队列与账本已就位(T14),
  T15 只需把动作面接上去 —— 不应出现第二条管线。
