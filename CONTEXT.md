...
- 快带 `npm test`(**24 文件 219 测试**)全绿;慢带 `npm run test:slow`
...
- **#27 已交付(T19)**:会话里多一段 `galfree-workflow` 的 system prompt section
  (顺序 / 闸门 / 完成判据 / 谁来做 / 怎么问现状),由 `src/service/playbook.ts` 渲染。
  判据写成**板上的字段路径**、闸门写成 `GATE`(`src/service/gates.ts`,接缝与指引的**唯一出处**),
  所以指引不可能跟接缝分叉 —— 字段改名守卫就红。**工具面只报真的注册了的入口**:
  没有入口的环节如实说"请人在工作台做",#28 补上工具后指引自动点名。
  懒注入(`ctx.inject(['systemPrompt'])`):宿主没这个席位就少一段提示,插件不崩、工具照常。
  契约见「agent 流程指引(T19)」节。**改 `lib/` 后要重启宿主才会生效**。
