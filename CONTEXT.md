...
- **环节零(T1–T7)、剧本环节(T8–T13)、图像渠道 + 任务队列(T14)、出图动作面(T15)、
  参考链一致性回路(T16)与音频接线(T17)已交付**。issues #9–#25 已关闭;还开着
  **#26(T18)+ #1(Spec)**。接缝契约成文:`docs/contracts/stage-zero.md`
  (含 T8–T17 各节 + 模板界面层 + 两种上游协议)+ `dialect-subset.md`。
  本会话交接见 `docs/handoff-2026-09-12-t17.md`。
- 快带 `npm test`(**22 文件 191 测试**)全绿;慢带 `npm run test:slow`
  (**4 文件 34 测试通过 + 1 跳过**,跳过的是真上游参考链验证:它花钱,默认不跑;
  慢带里新增了音频接线那条:真 SDK lint 认不认)。**发版前每次都要重跑**;
  慢带已改**串行**,而且跑它的时候别同时跑别的 vitest 进程(见过一次 0ms 全红的幻影失败,
  单独重跑即干净)。
...
- 遗留:人工验收还欠"点一次试玩看窗口能否正常退出"(无头环境验不了"玩通");
  ...
  见上)—— 建议先在设置里关掉该模型的「参考链」。
- 下一张票 **T18(本地发布 `build_dists`)**,之后 v1 收口。
