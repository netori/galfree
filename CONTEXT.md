...
- **语音接线(T26 / ADR-0013)从来没有在生产里跑通过 —— 2026-09-19 查清并修好**
  (commit `54e1f60`)。作者试玩反馈"听不到语音";655 个语音文件一个不差、还是**一句都不响**。
  两个前提**都没落地**,而且缺任何一个都是**静默无声**(引擎不报错、试玩技术上也照过):
  1. **项目里没有 `config.auto_voice`** —— 模板写这一行,但项目建得比它早 ⇒
     **老项目永远缺**(同 `build.name` / 界面补丁那两处:模板演进只作用于新建);
  2. **`stampDialogueIds` 从来没有生产调用者** —— **只在测试里被调用过**。于是剧本里
     一句显式 `id` 都没有,而没显式 id 时 Ren'Py 用**内容哈希**当标识符
     (`renpy/translation/__init__.py:337-357`),`auto_voice` 就去格式化一个不存在的文件名
     (`renpy/common/00voice.rpy:364-372`)。
     T26 当年的慢带验的是"**这个函数对**",**没验"生成那条路调了它"** ——
     与 T15(素材槽盖戳 / 快照回滚)、T33(建音频任务的入口)是**同一个模式**:
     **接缝已备、入口缺失**。这是第三次,值得当成一类问题看。
  **修法**:`generateScene` 盖章;新增 `galfree_voice_wiring`(read/apply)给老项目补课
  (逐场盖章 + 补那行配置,一个写批一个快照;手写文件只报不改);
  `dialogueRowsOf` / `VoiceBatchRow` 加 `stamped`,把"id 有值"与"引擎认这个名字"**分开**
  (光看清单发现不了这个坑 —— 派生的 id 照样有值)。
  **连带修掉两个真实缺陷**:
  - **`setDialogue` 会把 id 弄丢**:表单改台词时那一行是**重建**的,原有的 id 子句被悄悄丢掉
    ⇒ 改过的句子从此没声音。现在用 `idClauseTextOf` 把原子句原样接回去
    (`edit.test.ts` 那两条最小-diff 断言现在**同时守着这件事**)。
  - **音频池把语音全报成"未使用"**:`unused` 问的是"有没有 `play` 引用",而语音按设计
    不写 `play`(走 auto_voice)。实测 655 条全被误报。现在 `unused` 排除 `voice/`,
    但语音文件**仍在 `files` 里**(T29 的验收口径是"落盘 → 池里立刻有它")。
  **对真实项目的 apply**:53 个文件(52 场 + `options.rpy`),3,111 行对白盖章;
  两个女主角 657 条 id 对上 655 个语音文件(差的 2 条是 `"……"` 纯停顿,预期如此)。
  **审读戳完好**:approved 3 / 待复审 7,与盖章前**逐字相同** —— T26 那条
