...
- **环节零(T1–T7)、剧本环节(T8–T13)、图像渠道 + 任务队列(T14)与出图动作面(T15)已交付**。
  issues #9–#23 已关闭。接缝契约成文:`docs/contracts/stage-zero.md`(含 T8–T15 各节)+ `dialect-subset.md`。
- 快带 `npm test`(**18 文件 145**)全绿;慢带 `npm run test:slow`(**30**,含真钉版 SDK 的
...
- T15 的**动作面与 agent 工具面是同一条队列**(结构保证:两边都只是接缝的搬运工)。
  ...
  写批前的快照里,"可对比"因此有据可查。
- 遗留:工作台面板的人工验收(重启后看 T8–T15 的卡)已由真渲染截图回路覆盖渲染面,
  "装进真宿主窗口"由用户确认过;**从未用真上游跑过一次出图**(渠道配好后可验),
  参考链一致性回路属 **T16**;音频/发布(T17–T18)未开工。
