...
- **#28 已交付(T20)**:工具面从 10 个长到 **16 个**,七个环节**全部有 agent 入口**
  (建项目 / 设定集 / 场景编辑 / 音频接线 / 试玩 / 快照历史与回滚)。
  每个都是接缝的搬运工(同一个动作从面板与从工具走,落到同一份账本与推导)。
  **搬家(`relocateScene`)没有 agent 入口**:接缝上它 `#requireHuman`(它重写人的手写文件)——
  要放开先提 ADR 修订。**审读戳的两道守卫**:显式工具名清单 + 接缝对 agent 一律
  `stamp-forbidden`。AC1 的证据是一段**只经工具调用**的端到端回放(快带,夹具全是注入端口)。
