...
- 遗留:人工验收还欠"点一次试玩看窗口能否正常退出"(无头环境验不了"玩通");
  ...
  —— 建议在设置里关掉该模型的「参考链」;**老项目**(模板里没有 `build.name` 的那些,
  例如用户自己的 `hjm`)在发布时会被如实拦下,补一行 `define build.name = "<名字>"` 即可。
- **v1 之后**:若还要继续,候选是"参考链要公网地址"那一环(上游文件上传/图床)、
  老项目的一键补 `build.name`、UI 细节打磨。
