...
- **v3 方向(用户 2026-09-12 提出,ADR 已起草、等点头)**:**音乐自动生成 + 语音(TTS)自动生成
  + 封面/主菜单/窗口图标**。两张 ADR 是草稿(`docs/adr/0012-generation-channels.md` 三条生成线
  各自一条渠道、`docs/adr/0013-voice-wiring-by-dialogue-id.md` 语音接线走
  **对话 id + `config.auto_voice`**);它们在**放宽 v1 明确圈掉的边界**(契约「音频接线」节的
  "没有音乐生成、没有 TTS"、ADR-0005 环节 4/5),所以**没点头前不实现**。
  票在 GitHub:**#33**(ADR)/ **#34**(对话 id,是 TTS 的前置)/ **#35**(音频通道骨架)/
  **#36**(音乐)/ **#37**(TTS)/ **#38**(封面·主菜单·窗口图标)/ **#39**(游戏内界面换皮)。
