...
- **环节零(T1–T7)、剧本环节(T8–T13)、图像渠道 + 任务队列(T14)与出图动作面(T15)已交付**。
  issues #9–#23 已关闭;还开着 **#24(T16)/ #25(T17)/ #26(T18)+ #1(Spec)**。
  接缝契约成文:`docs/contracts/stage-zero.md`(含 T8–T15 各节 + 模板界面层 + 两种上游协议)+ `dialect-subset.md`。
  本会话交接见 `docs/handoff-2026-09-12-t14-t15.md`(含一批实测事实,别再重查)。
- 快带 `npm test`(**19 文件 162**)全绿;慢带 `npm run test:slow`(**31**,含真钉版 SDK 的模板
  启动、逐场生成闭环、从某场试玩、**界面层守卫**)在 2026-09-12 跑过并全绿 —— **发版前每次都要重跑**。
  慢带已改**串行**(并行会抢 SDK 与游戏窗口,造成假失败)。
- T14 把**写网关拓宽到二进制**(`WriteOp.content: string | Uint8Array | null`),版本戳
  一律按原始字节 —— 这是"素材经网关落盘"这条铁律的前置,不是旁路。
...
- 遗留:人工验收还欠"点一次试玩看窗口能否正常退出"(无头环境验不了"玩通");
  `alice` 还没进角色登记簿(warning);**参考链从未用真模型验过**(T16 带上);
  音频/发布(T17–T18)未开工。
- 下一张票 **T16(参考链一致性回路)**:差分批量任务要携正确链引用、槽位对比视图、
  拒收注记进任务历史;真模型链验证入慢带(T14 起就欠着的那条"真适配器"验证也归这里)。
